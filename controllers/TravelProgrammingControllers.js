...
var travelProgramming = models.TravelProgramming;
export function index(req,res) {
	travelProgramming.findAll()
	.then(result=>{
...
export function show(req,res) {
	travelProgramming.findById(req.params.id)
	.then(result=>{
		return res.json(response.success(result))
	}).catch(err=>{
		return res.json(response.error(err))
...
export function store (req, res) {
	travelProgramming.create(req.body)
	.then(result=>{
		return res.json(response.success(result))
	}).catch(err=>{
...
export function update (req, res) {
	travelProgramming.update(req.body,{
		where:{
			id:req.params.id
		}
	})
